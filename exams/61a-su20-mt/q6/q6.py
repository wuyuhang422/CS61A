...
def copycat(lst1, lst2):
    ...
    ['c', 'c', 'c']
    """
    def copycat_helper(lst1, lst2):
        if not lst1 or not lst2:
            return []
        if lst2[-1] >= 0:
            ans = copycat_helper(lst1[:-1], lst2[:-1]) + [lst1[-1] for i in range(lst2[-1])]
        else:
            ans = copycat_helper(lst1[:-1], lst2[:-1])[:lst2[-1]]
        return ans
    return copycat_helper(lst1[:min(len(lst1),len(lst2))], lst2[:min(len(lst1),len(lst2))])
