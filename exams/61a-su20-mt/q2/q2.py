...
def schedule(galaxy, sum_to, max_digit):
    ...
    def schedule_helper(galaxy, sum_sofar, index):
        if not galaxy and sum_sofar == sum_to:
            return [galaxy]
        elif not galaxy:
            return []
        elif galaxy[0] != '?':
            return [galaxy[0]+x for x in schedule_helper(galaxy[1:], sum_sofar+int(galaxy[0]), index)]
        ans = []
        for x in range(0, max_digit + 1):
            modified_galaxy = schedule_helper(galaxy[1:], sum_sofar+x, index)
            ans.extend([str(x)+y for y in modified_galaxy])
        return ans
