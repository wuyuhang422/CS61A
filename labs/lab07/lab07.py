...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        tmp = subseqs(s[1:])
        return tmp + insert_into_all(s[0], [x[:] for x in tmp])
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return a + insert_into_all(s[0], b)
    return subseq_helper(s, min(s)-1)
...
def make_glookup(class_assignments):
    ...
    now = 0
    total = 0
    def record(part, score):
        nonlocal now, total, class_assignments
        if part in class_assignments:
            total += class_assignments.pop(part)
        now += score
        return now/total
    return record
