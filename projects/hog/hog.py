...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    t = player_score%10 - opponent_score%10;
    if t < 0:
        t = -t
    return t == (opponent_score//10)%10
    # END PROBLEM 4
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        now = score1 if who else score0
        delta = now-last_score
        if delta > running_high:
            print(delta,'point(s)! That\'s the biggest gain yet for Player',who)
        return announce_highest(who,now,max(running_high,delta))
    return say
    # END PROBLEM 7
...
def run_experiments():
    ...
    if False:  # Change False to test always_roll(8)
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    delta = free_bacon(opponent_score);
    now = score + delta
    if is_swap(now,opponent_score):
        if now > opponent_score:
            return num_rolls
        elif now < opponent_score:
            return 0
    return num_rolls if delta < cutoff else 0  # Replace this statement
    # END PROBLEM 11


def final_strategy(score, opponent_score, cutoff=8):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    delta = free_bacon(opponent_score);
    if score + delta >= 100 and not is_swap(score + delta, opponent_score):
        return 0
    if is_swap(score + 1, opponent_score) and score + 1 < opponent_score:
        return 10
    if is_swap(score + delta, opponent_score) and score + delta + cutoff < opponent_score:
        return 0
