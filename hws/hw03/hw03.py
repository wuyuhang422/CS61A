...
def missing_digits(n):
    ...
    t1 = n%10
    t2 = (n//10)%10
    return missing_digits(n//10) + (0 if t1 == t2 else abs(t1-t2)-1)
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_helper(digits, x):
        if digits == 0 or x == 0:
            return 1
        ans = count_helper(digits-1, x);
        if x >= (2**digits):
            ans += count_helper(digits, x-(2**digits))
        return ans
        
        
    return count_helper(30, total)
