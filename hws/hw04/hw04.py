...
def balanced(m):
    ...
    if is_planet(m):
        return True
    larm,rarm = m[1:]
    ls,rs = end(larm), end(rarm)
    return total_weight(ls) * length(larm) == total_weight(rs) * length(rarm) and balanced(ls) and balanced(rs)
...
def replace_leaf(t, find_value, replace_value):
    ...
    def construct(t):
        ...
        return tree(label(t), [construct(x) for x in branches(t)])
    return construct(t)
...
def quadratic(x, a, b, c):
    ...
    def f(t):
        return a*t*t+b*t+c
    point = -b/(2*a)
    if upper_bound(x) <= point or lower_bound(x) >= point:
        y1 = f(lower_bound(x))
        y2 = f(upper_bound(x))
        y3 = f(upper_bound(x))
    else:
